...
namespace menu_utils
{
	...
	void set_color(bool white)
	{
		...
		if (white)
		{
			style.Colors[ImGuiCol_FrameBg] = ImColor(200, 200, 255);
			style.Colors[ImGuiCol_FrameBgHovered] = ImColor(220, 220, 255);
			style.Colors[ImGuiCol_FrameBgActive] = ImColor(230, 230, 255);
			style.Colors[ImGuiCol_Separator] = ImColor(180, 180, 255);
			style.Colors[ImGuiCol_CheckMark] = ImColor(134, 103, 181);
			style.Colors[ImGuiCol_SliderGrab] = ImColor(134, 103, 181);
			style.Colors[ImGuiCol_SliderGrabActive] = ImColor(134, 103, 181);
			style.Colors[ImGuiCol_ScrollbarBg] = ImColor(120, 120, 255);
			style.Colors[ImGuiCol_ScrollbarGrab] = ImColor(134, 103, 181);
			...
			style.Colors[ImGuiCol_Header] = ImColor(160, 160, 255);
			style.Colors[ImGuiCol_HeaderHovered] = ImColor(200, 200, 255);
			style.Colors[ImGuiCol_Button] = ImColor(180, 180, 255);
			style.Colors[ImGuiCol_ButtonHovered] = ImColor(200, 200, 255);
			style.Colors[ImGuiCol_ButtonActive] = ImColor(230, 230, 255);
...
void CMenu::Run()
{
	...
	ImGui::SetNextWindowSize(ImVec2(570.f, 530.f), ImGuiCond_Once);
	...
	if (m_Page == PAGE_GLOBALS_AIMBOT)
	{
		...
		const char* csz_aimbot_tabs[] = { "Undetected functional", "Unsafe" };
		...
		else
		{
			...
			{
				if (ImGui::SliderFloat("Rate of weapon firerate", &vars::aimbot::rate_on_firerate, 0.001f, 0.1f, "%.3f"));
				{
					m_pFeatures->SetupFirerate(vars::aimbot::rate_on_firerate);
				}
			}
			...
			std::string no_spread_label = vars::aimbot::no_spread == false ? "Enable no spread" : "Disable no spread";
			if (ImGui::Button(no_spread_label.c_str(), ImVec2(ImGui::GetWindowSize().x - 15.f, 40)))
			{
				vars::aimbot::no_spread = !vars::aimbot::no_spread;
				m_pFeatures->NoSpread(vars::aimbot::no_spread);
			}
			ImGui::PopStyleColor();
		}
	...
	else if (m_Page == PAGE_GLOBALS_VISUALS)
	{
		...
		if (ImGui::TreeNode("ESP"))
		{
			...
			ImGui::Checkbox("Deployment ESP", &vars::visuals::deployment_ESP);
	...
	ImGui::SameLine();
	ImGui::SetCursorPosX(ImGui::GetWindowSize().x - ImGui::CalcTextSize("by bulligan").x - 15.f);
	ImGui::Text("by bulligan");
	ImGui::PopFont();
